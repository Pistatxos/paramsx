...
- **Validación por parámetro.** Al cargar con `abac = True` y `obligatorias_vacias = False`, si a un parámetro le falta cualquiera de las tags obligatorias no se sube ni su valor ni sus tags, y se indica qué falta y en qué `parameter_name`. El resto del lote se sube con normalidad.
- **Flag `obligatorias_vacias`.** Con `True` se permite dejar vacías las tags obligatorias: el parámetro se sube igualmente y **las tags sin valor no se crean en AWS** (nunca se suben etiquetas vacías). El informe de carga lista qué parámetros han quedado con tags vacías. Con `False` (valor por defecto) se mantiene la validación bloqueante. Vaciar una tag que **ya tenía valor** en AWS sigue significando borrarla, en los dos modos. Cuando la carga queda a medias los ficheros no se borran y el backup se resincroniza con lo ya aplicado, así que la segunda pasada solo sube lo pendiente.
